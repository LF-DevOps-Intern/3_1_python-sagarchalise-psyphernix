...
def httpserver(flag):
    ...
    if flag != None: # checking if flag is given
        subprocess.call(["python3","-m","http.server"]) # starting http server using subprocess module
...
def data_save(data):
    ...
    '''
    try: #
        date = datetime.now().strftime("%Y%m%d%I%M%S%p") # date and time in format of YYYYMMDDHHMMSSAM
